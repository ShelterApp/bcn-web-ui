...
const Feedbacks = React.memo((props: FeedbacksProps) => {
  ...
  const [typeHandler, setTypeHandler] = useState<any>();
  ...
  const tabs = current_user.isAdmin
    ? ["APP_FEEDBACK", "SERVICE_FEEDBACK", "CLAIM_FEEDBACK", "ARCHIVE"]
    : ["SERVICE_FEEDBACK", "CLAIM_FEEDBACK", "ARCHIVE"];
  ...
  const tranformFeedbacks = () => {
    switch (selectedIndex) {
      ...
      case 1:
        if (current_user.isAdmin)
          return feedbacks.filter(f => !f.isArchive && f.type === "SERVICE");
        return feedbacks.filter(f => !f.isArchive && f.type === "CLAIM");
      case 2:
        if (current_user.isAdmin)
          return feedbacks.filter(f => !f.isArchive && f.type === "CLAIM");
