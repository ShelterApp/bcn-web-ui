...
const Chatbox = React.memo((props: ChatboxProps) => {
                    ...
                    switch (type) {
                        ...
                        case "CRISIS_LINES_LIST":
                            quickReplies = {
                                ...
                                type: "radio",
                                values: fields.data.listValue.values.map(
                                    (rep) => struct.decode(rep.structValue)
                                ),
                            };
                            break;
                        case "WEATHER":
    ...
    const renderMessageText = (props) => {
        ...
        const regex = /(https?:\/\/[^\s]+)/g; //detect HTTP,HTTPS
        const urls = currentMessage.text[0].match(regex) || [];
        return (
            <p
                className={clsx(
            ...
            >
                {!urls.length ? (
                    ...
                    <>
                        <a>{currentMessage.text[0].replace(urls[0], "")}</a>
    ...
    const ServiceItem = (props) => {
        ...
        const fullAddress = `${address1 || address2}, ${state}, ${city}`;
                ...
                <p className={clsx(classes.nameService)}>
                    <span>
                        <div className={clsx(classes.iconCircle)}>
                            <FontAwesomeIcon
                                icon={isYouthKid ? faChild : faUsers}
                            />
                        </div>
                    </span>
                    {withAge}
                ...
                {services.map((tempSer) => {
                    return (
                        <a
                            href={
                                tempSer.website
                                    ? parseURL(tempSer.website)
                                    : "#"
                            }
                            className={clsx(
                                classes.btnpill,
                                classes.btnpill_line
                            )}
