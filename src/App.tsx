...
import { initApi } from "@fywinnv/core/dist/apis";
...
const App = ({ history, dispatch }: AppProps) => {
    ...
    const getLocation = (showPosition, callback) => {
        ...
        const cb = () => {
            if (sessionStorage.getItem("@fywinnv_block_location") === null) {
                sessionStorage.setItem(
                    "@fywinnv_block_location",
                    JSON.stringify(true)
                );
            }
            callback();
    ...
    useBeforeFirstRender(() => {
        ...
        if (!sessionStorage.getItem("@fywinnv_current_location")) {
    ...
    React.useEffect(() => {
        ...
        if (
            Cookie.get("@fywinnv_alreadyaccess") === undefined ||
            !Boolean(JSON.parse(Cookie.get("@fywinnv_alreadyaccess")))
        ) {
            if (
